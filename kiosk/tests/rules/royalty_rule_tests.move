...
fun test_default_flow_1_min_10_000() {
    ...
    royalty_rule::pay(&mut policy, &mut request, payment);
    policy::confirm_request(&policy, request);
    assert_eq!(test::wrapup(policy, cap, ctx), 10_000);
}
...
fun test_default_flow_20_min_10_000() {
    ...
    royalty_rule::pay(&mut policy, &mut request, payment);
    policy::confirm_request(&policy, request);
    assert_eq!(test::wrapup(policy, cap, ctx),  20_000);
}

#[test, expected_failure(abort_code = kiosk::royalty_rule::EIncorrectArgument)]
