...
module quorum_upgrade_policy::quorum_upgrade_policy_test {
    ...
    #[test]
    fun quorum_upgrade_propose_upgrade_v2_ok() {
        let test = test::begin(ADDRESS_1);
        let digest: vector<u8> = x"0123456789";
        let quorum_upgrade_cap = get_quorum_upgrade_cap(3, 5, &mut test);
    ...
    fun quorum_upgrade_authorize_upgrade_v2_bad_signer() {
        ...
        propose_upgrade_v2_with_metadata(ADDRESS_1, &quorum_upgrade_cap, digest, &mut test);
        let quorum_upgrade_cap_1 = get_quorum_upgrade_cap(3, 5, &mut test);
        propose_upgrade_v2_with_metadata(ADDRESS_2, &quorum_upgrade_cap_1, digest, &mut test);
    ...
    fun quorum_upgrade_authorize_upgrade_v2_bad_voter_cap() {
        ...
        propose_upgrade_v2_with_metadata(ADDRESS_1, &quorum_upgrade_cap, digest, &mut test);
        let quorum_upgrade_cap_1 = get_quorum_upgrade_cap(3, 5, &mut test);
        propose_upgrade_v2_with_metadata(ADDRESS_2, &quorum_upgrade_cap_1, digest1, &mut test);
    ...
    fun quorum_upgrade_v2_invalid_proposer_metadata_change() {
        ...
        quorum_upgrade_policy::add_metadata(&mut proposed_upgrade, string::utf8(b"metadata_key"), b"metadata_info", ctx(&mut test));
        quorum_upgrade_policy::share_upgrade_object(proposed_upgrade);
    ...
    fun quorum_upgrade_v2_valid_proposer_metadata_change() {
        ...
        test::next_tx(&mut test, ADDRESS_1);
        let proposed_upgrade = quorum_upgrade_policy::propose_upgrade_v2(&quorum_upgrade_cap, digest, ctx(&mut test));
        quorum_upgrade_policy::add_all_metadata(&mut proposed_upgrade, metadata_map, ctx(&mut test));
        quorum_upgrade_policy::add_metadata(&mut proposed_upgrade, string::utf8(b"metadata_key"), b"metadata_info_new", ctx(&mut test));
        quorum_upgrade_policy::share_upgrade_object(proposed_upgrade);
    ...
    #[test]
    fun quorum_upgrade_v2_add_all_metadata() {
        let digest: vector<u8> = x"0123456789";
        let metadata_map = vec_map::empty<string::String, vector<u8>>();
        vec_map::insert(&mut metadata_map, string::utf8(b"metadata_key"), b"metadata_info");
        vec_map::insert(&mut metadata_map, string::utf8(b"metadata_key_2"), b"metadata_info_2");
        ...
        let metadata_map: &VecMap<string::String, vector<u8>> = quorum_upgrade_policy::get_metadata(&mut proposal);
        assert!(*(vec_map::get(metadata_map, &string::utf8(b"metadata_key"))) == b"metadata_info_new", 0);
    ...
    fun quorum_upgrade_v2_no_metadata_perform_upgrade_ok() {
        ...
        let test = test::begin(ADDRESS_1);
        let quorum_upgrade_cap = get_quorum_upgrade_cap(3, 5, &mut test);
        propose_upgrade_v2_no_metadata(ADDRESS_1, &quorum_upgrade_cap, digest, &mut test);
        vote(@0x100, &mut test);
        vote(@0x101, &mut test);
        vote(@0x104, &mut test);
        perform_upgrade(ADDRESS_1, &mut quorum_upgrade_cap, &mut test);
        ...
        let test = test::begin(ADDRESS_2);
        let quorum_upgrade_cap = get_quorum_upgrade_cap(8, 10, &mut test);
        propose_upgrade_v2_no_metadata(ADDRESS_2, &quorum_upgrade_cap, digest, &mut test);
        vote(@0x100, &mut test);
        vote(@0x101, &mut test);
        vote(@0x104, &mut test);
        vote(@0x105, &mut test);
        ...
        let test = test::begin(@0x3);
        let quorum_upgrade_cap = get_quorum_upgrade_cap(3, 9, &mut test);
        propose_upgrade_v2_no_metadata(@0x3, &quorum_upgrade_cap, digest, &mut test);
        vote(@0x100, &mut test);
        vote(@0x101, &mut test);
        vote(@0x104, &mut test);
        vote(@0x105, &mut test);
    ...
    fun quorum_upgrade_v2_with_metadata_perform_upgrade_ok() {
        ...
        let test = test::begin(ADDRESS_2);
        let quorum_upgrade_cap = get_quorum_upgrade_cap(8, 10, &mut test);
        propose_upgrade_v2_with_metadata(ADDRESS_2, &quorum_upgrade_cap, digest, &mut test);
        vote(@0x100, &mut test);
        vote(@0x101, &mut test);
        vote(@0x104, &mut test);
        vote(@0x105, &mut test);
        ...
        let test = test::begin(@0x3);
        let quorum_upgrade_cap = get_quorum_upgrade_cap(3, 9, &mut test);
        propose_upgrade_v2_with_metadata(@0x3, &quorum_upgrade_cap, digest, &mut test);
        vote(@0x100, &mut test);
        vote(@0x101, &mut test);
        vote(@0x104, &mut test);
        vote(@0x105, &mut test);
    ...
    fun propose_upgrade_v2_no_metadata(
        sender: address, 
        quorum_upgrade_cap: &QuorumUpgradeCap, 
        digest: vector<u8>,
        test: &mut Scenario,
    ...
    fun propose_upgrade_v2_with_metadata(
        sender: address, 
        quorum_upgrade_cap: &QuorumUpgradeCap, 
        digest: vector<u8>,
        test: &mut Scenario,
        ...
        test::next_tx(test, sender);
        let proposed_upgrade = quorum_upgrade_policy::propose_upgrade_v2(quorum_upgrade_cap, digest, ctx(test));
        quorum_upgrade_policy::add_metadata(&mut proposed_upgrade, string::utf8(b"metadata_key"), b"metadata_info", ctx(test));
        quorum_upgrade_policy::share_upgrade_object(proposed_upgrade);
    }
